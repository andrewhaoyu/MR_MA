...
#check for fix tau as 0
load("./result/simulation/LD/WMR_result_chr22_grid_fixtau0.rdata")
head(result)
#subset result to WMR
result = result %>% filter(method=="WMR")
#v_vec = 1 as no pleiotropic effect
  ...
  for(i in 1:2){
    for(l in 1:3){
      ...
      p2 = ggplot(result.sub,aes(x = index,y =  em_se,fill=p_vec))+
        ...
        guides(fill=guide_legend(title="P-value"))
      # print(p2)
      png(filename = paste0("./result/simulation/LD/chr22_figures_fixtau0/fidxed_r2_different_p_value_r_",r,"_beta_",i,"_cau_",l,"_LDgrid.png"),width = 15, height = 8, res=300,units  = "in")
      grid.arrange(p1,p2,p3,
                   layout_matrix=rbind(c(1, 2),
                                       c(3, 3)),
                   top=textGrob(paste0("Beta = ",beta_vec[i],", clumping R2 = ",r),gp=gpar(fontsize=24,font = 2)))
