...
result.list = list()
temp = 1
files = dir(cur.dir,pattern="_fixtau_",full.names = T)
for(i in 1:2){
  for(l in 1:3){
    for(rep_ind in 1:100){
      file = paste0(cur.dir,"/WMR_result_chr22_beta_",i,"_rho_",l,"_rep_ind_",rep_ind,"_fixtau_.rdata")
      if(file %in% files==T){
        load(file)
        result$l_vec = rep(l,nrow(result))
        result$rep_ind = rep(rep_ind,nrow(result))
...
sum.result = rbindlist(result.table.list)
result = sum.result
save(result,file = paste0(cur.dir,"WMR_result_chr22_grid_fixtau0.rdata"))
