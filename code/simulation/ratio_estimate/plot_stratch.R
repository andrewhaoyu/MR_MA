...
for(j in 1:length(gamma_vec)){
  ...
  png(filename = paste0("./result/simulation/ratio_estimate/qq_plot_true_plug/betadis_alpha_",gamma,".png"),res=300,width=12,height = 6,unit="in")
...
for(j in 1:length(gamma_vec)){
  ...
  z_gamma <- rnorm(n.simu,mean = sqrt(n)*gamma,sd = 1)
  ...
  for(k in 1:length(alpha_est_vec)){
    alpha_est = alpha_est_vec[k]
    z_Gamma <- rnorm(n.simu)
    z_gamma <- rnorm(n.simu,mean = sqrt(n)*alpha_est,sd = 1)
    true_distribution2 =  z_Gamma/sqrt(1+z_Gamma^2/z_gamma^2)
    dot2 = dot
    for(i in 1:length(dot)){
      idx <- which(true_distribution2<=q_true[i])
      dot2[i] = length(idx)/length(true_distribution2)
  ...
  png(filename = paste0("./result/simulation/ratio_estimate/qq_plot_true_plug/ratiodis_alpha_",gamma,".png"),res=300,width=12,height = 6,unit="in")
  grid.arrange(p[[1]],p[[2]],p[[3]],p[[4]],
               nrow=1)
  dev.off()
}
