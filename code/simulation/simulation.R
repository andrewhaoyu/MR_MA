...
#sigma_beta_est = rep(0,times)
n.test = 1000
G_ori = matrix(rbinom((n+n.test)*100,1,MAF),n+n.test,100)
...
# q <- 256
# mean(TwoStage_est[1:q])-beta_M
# mean(IVW_est[1:q])-beta_M
# mean(IVWs_est[1:q])-beta_M
# mean(cover_TwoStage_est[1:q])
# mean(cover_IVW_est[1:q])
# mean(cover_IVWs_est[1:q])
# var(TwoStage_est[1:q])
# var(IVW_est[1:q])
# var(IVWs_est[1:q])
# mean(sigma_TwoStage[1:q])
# mean(sigma_IVW[1:q])
# mean(sigma_IVWs[1:q])
# mean(twostage.nsnps[1:q])
# mean(twostage.prop[1:q])
# mean(IVW.nsnps[1:q])
# mean(IVW.prop[1:q])
